...
| 4 | ―　 (Not Used) |
| 5 | Multi User Mode with Display Manager (X윈도를 사용하는 다중 사용자 모드) |
| 6 | Reboot |
...
- RAID 2
     - 오류 정정을 위해 해밍코드(오류 정정 부호 = ECC)를 사용. 비트 단위에 해밍 코드를 적용.
...
> 대게 d가 붙는다.
- standalone / (x)inetd / systemd의 온디맨드 방식
...
Sever / Client Model = 서버 클라이언트 모델
...
- **프레임을 모든 장비에 전송**하는 플러딩(flooding)을 함.

### 브리지
- 모든 수신 프레임을 버퍼에 저장하고, MAC 주소에 따라 **프레임을 해당 포트로 전송하는 장비**
...
### 라우터
- 송신 측의 패킷을 목적지까지 전달하기 위해 **최적의 경로를 찾아 데이터를 포워딩**함.
- 라우팅 테이블을 통해 경로를 설정함.
